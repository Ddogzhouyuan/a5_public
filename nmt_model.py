...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(input_size=word_embed_size, hidden_size=self.hidden_size, bias=True,
                               bidirectional=True)
        self.decoder = nn.LSTMCell(input_size=self.hidden_size + word_embed_size, hidden_size=self.hidden_size, bias=True)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        enc_hidden, dec_init_state = self.encode(source_padded_chars, source_lengths)
        enc_mask = self.generate_sent_masks(enc_hidden, source_lengths)
        combined_outputs = self.decode(enc_hidden, enc_mask, dec_init_state, target_padded_chars)
        ### END YOUR CODE
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        Xpacked = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(Xpacked)
        enc_hiddens_packed = pad_packed_sequence(enc_hiddens, batch_first=True)
        enc_hiddens = enc_hiddens_packed[0]
